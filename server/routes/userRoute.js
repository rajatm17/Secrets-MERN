...
userRouter.post('/signup', async (req, res) => {
  ...
  if (!username || !password) {
    res.status(202).json({ message: 'Username and password required' });
  } else {
...
userRouter.post('/login', async (req, res, next) => {
  ...
  if (!username || !password) {
    res.status(202).json({ message: 'Username and password is required' });
  } else {
...
userRouter.post('/recovery', async (req, res, next) => {
  ...
  if (!username) {
    res.status(202).json({ message: 'Username is required' });
  } else {
    ...
    if (userExists) {
      res
        .status(201)
        .json({ username: username, password: userExists.password });
    } else {
